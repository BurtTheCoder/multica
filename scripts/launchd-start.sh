...
# The API logs every daemon claim poll at debug level; info keeps server.log
# readable and small.
export LOG_LEVEL="${LOG_LEVEL:-info}"
FRONTEND_PORT="${FRONTEND_PORT:-3001}"

# Truncate any log that has grown past the cap. launchd owns stack.*.log and
# the children append to the others, so truncating in place is safe.
LOG_CAP_BYTES=$((200 * 1024 * 1024))
rotate_logs() {
  local f size
  for f in "$LOG_DIR"/*.log /Users/orie/.multica/daemon.log; do
    [ -f "$f" ] || continue
    size=$(stat -f %z "$f" 2>/dev/null || echo 0)
    if [ "$size" -gt "$LOG_CAP_BYTES" ]; then
      : > "$f"
      echo "[$(date '+%Y-%m-%dT%H:%M:%S%z')] truncated $f (${size} bytes)" >&2
    fi
  done
}
rotate_logs
...
while true; do
  ...
  loop_ticks=$((${loop_ticks:-0} + 1))
  if [ $((loop_ticks % 300)) -eq 0 ]; then
    rotate_logs
  fi
