...
export class ApiClient {
  ...
  private async fetchRaw(
    ...
    if (!res.ok) {
      ...
      // 401 is part of normal startup on public routes: the web shell probes
      // /api/me to discover whether an HttpOnly session exists, so it is not
      // an error worth a red console entry.
